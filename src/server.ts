...
async function startServer() {
    ...
    try {
        for (const [index, specCfg] of config.specConfigs.entries()) {
            console.error(`Loading spec [${index + 1}/${config.specConfigs.length}] from: ${specCfg.specPath}`);
        ...
        console.error('Failed to initialize/mapping OpenAPI specifications. Server cannot start.', error);
        process.exit(1);
    }

    if (mappedTools.length === 0) {
        console.error('No tools were mapped from the configured specs based on current configuration/filtering.');
    }
    ...
    const server = new McpServer({
        name: config.specConfigs.length > 1
    ...
    const registerTool: RegisterToolFn = (name, definition, handler) => {
        if (registeredToolNames.has(name)) {
            console.error(`Skipping duplicate tool name: ${name}. Use 'toolPrefix' in specs config to avoid collisions.`);
            return;
        }
        ...
        try {
            ...
            if (definition.description) {
                (server.tool as any)(name, definition.description, paramsSchema, async (toolParams: any) => handler(toolParams));
            } else {
