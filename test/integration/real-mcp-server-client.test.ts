...
jest.mock('../../src/apiClient', () => ({
  executeApiCall: jest.fn().mockImplementation(async (apiCallDetails) => {
    ...
    if (op === 'getPrice' || apiCallDetails.pathTemplate === '/apiv2/price') {
      return { success: true, data: testConfig.mockResponses.getPrice, statusCode: 200, headers: new Headers() };
    }
    if (op === 'getPools' || apiCallDetails.pathTemplate === '/apiv2/pools') {
      return { success: true, data: testConfig.mockResponses.getPools, statusCode: 200, headers: new Headers() };
    }
    return { success: true, data: { code: 0, msg: 'success', data: {} }, statusCode: 200, headers: new Headers() };
  }),
}));
...
describe('MCP Tool Integration Tests with Direct Handler Calls (SUN.IO)', () => {
  ...
  it('propagates API client errors through handlers', async () => {
    ...
    const handler = toolHandlers['getPrice'];
    await expect(handler({ params: { symbol: 'SUN' }, request: { id: 'req-3' } })).rejects.toThrow('Test error message');
  });
});
